<div class="flex flex-col md:flex-row justify-between">
    <div class="ml-8 mt-8 w-max text-white leading-tight">
        <h1 class="mb-0 text-4xl leading-tight">Quarterpie</h1>
        <div class="flex w-full justify-between mb-1.5 gap-6">
            <h2 class="text-xl leading-none">PER  AUS</h2>
            <div class="flex items-center gap-4">
                <a class="text-xl leading-none underline-offset-2 hover:underline" href="/about">/about</a>

            </div>
        </div>
        <div class="flex w-full justify-between mb-1">
            <h2 class="text-xl leading-none">Quarterpie</h2>
            <h2 class="text-xl leading-none">Labs</h2>
        </div>
        <div class="flex w-full justify-between mb-1">
            <img
                class="w-12 h-12 my-auto"
                src="/branding/logo_white.png"
                alt="Quarterpie Labs Logo"
            />
            <div class="leading-none">
                <h5 class="text-md mb-0.5">creativity</h5>
                <h5 class="text-md mb-0.5">engineered</h5>
                <h5 class="text-md mb-0.5">8906.96050</h5>
            </div>
        </div>
        <h2 class="text-xl leading-none">EXT_Landing.Page</h2>
    </div>

    <div class="text-white ml-8 mr-8 md:ml-0 md:mr-8 mt-16 w-auto md:w-1/4 md:max-w-96 md:min-w-96 md:mt-8">
        <div class="h-1 bg-white mb-5"></div>
        <a class="flex justify-between items-center mb-6" href="/about">
            <div>
                <h2 class="text-xl leading-tight">About</h2>
                <h2 class="text-4xl leading-none">Me</h2>
            </div>
            <div class="">
                <img
                    class="w-12 h-12"
                    src="/branding/about_me.svg"
                    alt="About Me"
                />
            </div>
        </a>
        <div class="h-1 bg-white mb-5"></div>
        <a class="flex justify-between items-center mb-6" href="/projects">
            <div>
                <h2 class="text-xl leading-tight">Project</h2>
                <h2 class="text-4xl leading-none">Showcase</h2>
            </div>
            <div class="">
                <img
                    class="w-12 h-12"
                    src="/branding/m86.svg"
                    alt="Projects"
                />
            </div>
        </a>
        <div class="h-1 bg-white mb-5"></div>
        <div class="flex justify-between items-center mb-6">
            <div>
                <h2 class="text-xl leading-tight">Quarterpie</h2>
                <h2 class="text-4xl leading-none">Labs</h2>
            </div>
            <div class="">
                <img
                    class="w-12 h-12"
                    src="/branding/logo_white.png"
                    alt="Quarterpie Labs Logo"
                />
            </div>
        </div>
        <div class="h-1 bg-white mb-6"></div>
        <div class="flex justify-between items-center mb-6">
            <div>
                <h2 class="text-xl leading-tight">Personal</h2>
                <h2 class="text-4xl leading-none">Blog</h2>
            </div>
            <div class="">
                <img
                    class="w-12 h-12"
                    src="/branding/personal_blog.svg"
                    alt="Personal Blog"
                />
            </div>
        </div>
        <div class="h-1 bg-white"></div>
    </div>
</div>

<div class="absolute right-0 bottom-2 left-2 flex pr-2">
    <div class="mr-5 mt-auto">
        <img
            class="h-8 w-auto my-auto"
            src="/branding/barcode.svg"
            alt="PDF417 Barcode"
        />
    </div>
    <div class="relative flex-1 min-w-0">
        <div class="w-max">
            <div class="flex justify-between gap-4">
                <div class="">
                    <h5 class="text-[0.5rem] mb-0.5 text-white leading-none">RAAVH74</h5>
                    <h5 class="text-[0.5rem] mb-0.5 text-white leading-none">EXTENSOR)</h5>
                </div>
                <div class="mb-0.5">
                    <h5 class="text-[0.5rem] mb-0.5 text-white leading-none text-right">IRUSCU</h5>
                    <h5 class="text-[0.5rem] mb-0.5 text-white leading-none">DIGITORUIM</h5>
                </div>
            </div>
            <div class="absolute left-0 right-0 h-0 border-t border-dashed border-white"></div>
            <div class="mt-1">
                <img
                    class="h-3 w-full"
                    src="/branding/cir_rec.svg"
                    alt="cirrec"
                />
            </div>
        </div>
    </div>
</div>